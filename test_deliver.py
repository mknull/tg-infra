...
class TestDeliverRouting(unittest.TestCase):
    ...
    def test_routes_job_match_to_telegram(self, mock_send):
    ...
    def test_routes_weekly_report_to_email(self, mock_env, mock_token, mock_send):
        from lib import deliver
    ...
    def test_returns_none_on_delivery_failure(self, mock_send):
        from lib import deliver
        result = deliver("job_match", "content")
        self.assertIsNone(result)

    @patch("lib.send_telegram", return_value="msg_42")
    def test_writes_ref_map_on_telegram_success(self, mock_send):
    ...
    def test_no_ref_map_on_failure(self, mock_send):
        ref_map = STATE_DIR / "ref-map.jsonl"
        ref_map.unlink(missing_ok=True)
        from lib import deliver
